...
def orderItemsLeftToRight(items: list[Item]) -> list[Item]:
    """Return items ordered by position from left to right.

    For items sharing the same position, the original relative order is kept.
    """
    position_rank = {
        "left": 0,
        "middle": 1,
    ...
    return sorted(
        items,
        key=lambda obj: position_rank.get(
            getattr(obj, "position", ""),
            len(position_rank),
        ),
    )
...
def generateSimplePlan(items: list[Item]):
    ...
    openDishwasher = Task("open the dishwasher", "open_dw", open_dw_actions, None)
    ...
    for object in items:
        load_task = Task(f"load the {object.name}", "load", [], object)
    ...
    closeDishwasher = Task("close the dishwasher", "close_dw", close_dw_actions, None)
